...
REM Developement script:
REM Goals: 1. Add direct text input of tank ingame code
REM        2. Only back up the very first time (i.e. the very original file)
REM
...
REM Tank Gunnery Range_Mod
if EXIST "!script_path!data\k42\loc_rus\levels\LEVELS\SCRIPTS\cm_users\Tank Gunnery Range_scripts.engscr" (
	set "target_script=!script_path!data\k42\loc_rus\levels\LEVELS\SCRIPTS\cm_users\Tank Gunnery Range_scripts.engscr"
	)
...
if [%1]==[] (
	...
	if EXIST "!textFile!" (
		...
		echo.
		echo No file dropped and default mission not installed.
		echo.
		echo Usage:
...
echo   missing in the attached database, and for new units introduced by future JCM or 
echo   ITM versions. Availability of units depend on which mod version you have installed.
echo.
echo Any selection or entry is substituted as typed into the dropped .engscr file, or, 
echo   one of the default missions. Whether there is such a unit in your 
echo   activated mod is not checked. Be warned^^! The mission may crash if you choose
echo   or enter a non-existent tank code, or, if mission specific requirements are not
...
echo   1. by running the batch file again and choose restore (for the Tank Gunnery Range)
